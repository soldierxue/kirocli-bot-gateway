...
│         ▼                ▼                ▼                      │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐              │
│  │  kiro-cli   │  │  kiro-cli   │  │  kiro-cli   │  每聊天独立   │
│  │  (聊天 A)   │  │  (聊天 B)   │  │  (聊天 C)   │  实例        │
│  └─────────────┘  └─────────────┘  └─────────────┘              │
└─────────────────────────────────────────────────────────────────┘
```
...
- **⏰ 定时任务**：通过 `/cron add` 调度周期性任务，支持间隔和 cron 表达式
- **📋 任务编排**：将复杂任务分解为步骤并行执行（`/task run`）
- **💓 心跳检查**：周期性后台巡检，支持静默时段
- **📏 上下文监控**：自动上下文使用率预警（75%/90%），支持 `/compact` 压缩
- **⚡ 备用模型**：速率限制或容量不足时自动切换备用模型
- **🔄 配置热重载**：`.env` 变更每 30 秒自动检测（无需重启）
- **💬 聊天风格**：简洁自然的回复——不说"好问题！"之类的客套话
- **👋 首次引导**：首次使用时自然引导，自动保存到记忆
...
所有命令在两个平台都可用，但输入方式不同：

| 平台 | 使用方式 |
|------|----------|
| **Discord** | 原生斜杠命令，带自动补全 — 输入 `/` 即可看到命令提示 |
| **飞书** | 作为普通文本消息发送（如直接发送 `/project ls`） |
...
| 命令 | 说明 |
|------|------|
| `/cron add "名称" "消息" --every 3600` | 添加周期性任务（间隔模式） |
| `/cron add "名称" "消息" --schedule "0 9 * * 1-5"` | 添加 cron 表达式任务 |
| `/cron list` | 列出所有定时任务 |
| `/cron pause <id>` | 暂停任务 |
| `/cron resume <id>` | 恢复任务 |
| `/cron remove <id>` | 删除任务 |
...
| 命令 | 说明 |
|------|------|
| `/compact` | 压缩上下文窗口（为更长对话释放空间） |
| `/usage` | 查看 API 用量和额度 |
| `/tools` | 查看可用工具和权限 |
| `/mcp` | 查看已加载的 MCP 服务器 |
| `/clear` | 清除对话历史 |

未识别的 `/命令` 会自动转发给 kiro-cli 尝试执行。上下文使用率会被自动监控——当上下文窗口达到 75% 或 90% 时会显示提醒。

**网关管理：**

| 命令 | 说明 |
|------|------|
| `/cli status` | 查看所有 kiro-cli 实例、上下文使用率、定时任务和活跃任务 |
| `/cli restart` | 停止所有 kiro-cli 实例并重启后台（会话在下次消息时自动恢复） |
